...
{
    public class EduAppraiseService : RepositoryFactory<EduAppraiseEntity>, IEduAppraiseService
    {
        public IEnumerable<EduAppraiseEntity> GetList(string deptid, string eduid)
        {
            ...
            //{
            //    query = query.Where(x => x.AppraisePeopleId == userid);
            //}
            return query.OrderByDescending(x => x.CreateDate).ToList();
        }

        public IEnumerable<EduAppraiseEntity> GetPageList(string deptid, int page, int pagesize, out int total)
        {
            //string deptid = OperatorProvider.Provider.Current().DeptId;
            var query = this.BaseRepository().IQueryable();
            total = query.Count();
        ...
        public EduAppraiseEntity GetEntity(string keyValue)
        {
        ...
        /// <returns></returns>
        public void SaveForm(string keyValue, EduAppraiseEntity entity)
        {
