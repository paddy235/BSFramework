    ...
    public class IMContentService : RepositoryFactory, IMsgContentService
    {
        ...
        public IEnumerable<IMReadModel> GetList(Pagination pagination, string userId, string sendId,string flag = "1")
        {
            ...
            if (!userId.IsEmpty())
            {
                ...
                {
                    strSql.AppendFormat(" AND ( T.UserId = '{0}' or T.UserId = '{1}' )",userId.ToString(), sendId.ToString());
                }
                else
                {
                    strSql.AppendFormat(" AND T.UserId = '{0}'",userId.ToString());
                }
            }
            //信息发送者
            if (!sendId.IsEmpty())
            {
                ...
                {
                    strSql.AppendFormat(" AND ( T.SendId = '{0}' or T.SendId =  '{1}' )", sendId.ToString(), userId.ToString());
                }
                else
                {
                    strSql.AppendFormat(" AND T.SendId = '{0}'",sendId.ToString());
                }
            }
...
select t.sendid id,t.createdate timestamp,t.createusername username,'http://10.36.0.170/ERCHTMS/Content/images/on-line.png' avatar,t.msgcontent content from IM_CONTENT t
where t.sendid='{0}'
union
 select t.toid id,t.createdate timestamp,t.receivername username,'http://10.36.0.170/ERCHTMS/Content/images/on-line.png' avatar,t.msgcontent content from IM_CONTENT t
 where t.toid='{1}'
 )
 order by timestamp asc",sendId,toId);
        ...
        public IEnumerable<IMReadModel> GetListByGroupId(Pagination pagination, string queryJson)
        {
            ...
            {
                strSql.AppendFormat(" AND t.ToId = '{0}'",queryParam["groupId"].ToString());
            }
        ...
        public IEnumerable<IMReadNumModel> GetReadList(string userId)
        {
            ...
            if (!userId.IsEmpty())
            {
                strSql.AppendFormat(" AND ( t1.UserId = '{0}' OR t1.SendId = '{0}' )",userId);
            }
            strSql.Append(@" GROUP BY t1.SendId, t1.UserId
                             ORDER BY MAX (t1.CreateDate) DESC");
            return this.BaseRepository().FindList<IMReadNumModel>(strSql.ToString());
        }
        /// <summary>
        /// 获取某用户某种消息的总数
        /// </summary>
        ...
        public DataTable GetReadAllNum(string userId, string status)
        {
            ...
            if (!status.IsEmpty())
            {
                strSql.AppendFormat(" AND ReadStatus = '{0}'", status);
            }
            //接收者Id
            if (!userId.IsEmpty())
            {
        ...
        public int Update(string userId, string sendId, string status)
        {
            ...
            if (!status.IsEmpty())
            {
                strSql.AppendFormat(" SET ReadStatus = '{0}'",status);
            }
            strSql.Append(" WHERE 1 = 1 ");
            //发送者
            if (!sendId.IsEmpty())
            {
                strSql.AppendFormat(" AND SendId = '{0}'", sendId);
            }
            //接收者
            if (!userId.IsEmpty())
            {
