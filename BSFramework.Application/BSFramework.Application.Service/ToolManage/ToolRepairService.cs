...
{
    public class ToolRepairService : RepositoryFactory<ToolRepairEntity>, IToolRepairService
    {
        #region 获取数据
        /// <summary>
        /// 获取列表
        ...
        public IEnumerable<ToolRepairEntity> GetPageList(string from, string to, int page, int pagesize, out int total)
        {
            ...
            if (!string.IsNullOrEmpty(from))
            {
                ...
                query = query.Where(x => x.RepairDate >= f);
            }
            if (!string.IsNullOrEmpty(to))
            {
                DateTime t = Convert.ToDateTime(to).AddDays(1);
                query = query.Where(x => x.RepairDate < t);
            }
            total = query.Count();
            if (page < 0)
            {
