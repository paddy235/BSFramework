    ...
    public class WFProcessInstanceService : RepositoryFactory, WFProcessInstanceIService
    {
        ...
        public DataTable GetPageList(string userid, Pagination pagination, string queryJson, string type)
        {
            try
            {
                ...
                {
                    strSql.Append(string.Format(" AND w.CreateUserId = '{0}' ", userid));
                }
        ...
        public DataTable GetToMeBeforePageList(string userid, Pagination pagination, string queryJson)
        {
            try
            {
                ...
                strSql.Append(string.Format(@" AND ( w.MakerList LIKE '{0}' or  w.MakerList = '1' ", userid));
        ...
        public DataTable GetToMeAfterPageList(string userid, Pagination pagination, string queryJson)
        {
            try
            {
                ...
                strSql.Append(string.Format(@"SELECT
                                ...
                                WHERE
	                                w.EnabledMark = 1 AND w2.CreateUserId = '{0}' And w.CreateUserId != '{0}' ", userid));
                var parameter = new List<DbParameter>();
                var queryParam = queryJson.ToJObject();
            ...
            try
            {
                ...
                switch (state)
                {
                    case 0:
                        ...
                        content = "【暂停】" + username + "暂停了一个流程进程【" + entity.Code + "/" + entity.CustomName + "】";
                        break;
                    case 1:
                        ...
                        content = "【启用】" + username + "启用了一个流程进程【" + entity.Code + "/" + entity.CustomName + "】";
                        break;
                    case 2:
                        entity.isFinish = 2;
                        content = "【召回】" + username + "召回了一个流程进程【" + entity.Code + "/" + entity.CustomName + "】";
                        break;
                }
