...
using BSFramework.Util.FlowWork;
using System;
...
namespace BSFramework.Application.Service.FlowManage
{
    ...
    public class WFRuntimeService : RepositoryFactory, WFRuntimeIService
    {
        ...
        /// <returns></returns>
        public bool CreateInstance(string userid, string username, Guid processId, string schemeInfoId, WFProcessInstanceEntity wfProcessInstanceEntity, string frmData = null)
        {
            ...
            try
            {
                ...
                wfProcessInstanceEntity.MakerList = (wfruntime.GetStatus() != 4 ? GetMakerList(wfruntime, userid) : "");//当前节点可执行的人信息
                ...
                processOperationHistoryEntity.Content = "【创建】" + username + "创建了一个流程进程【" + wfProcessInstanceEntity.Code + "/" + wfProcessInstanceEntity.CustomName + "】";
                #endregion
        ...
        /// <returns></returns>
        public bool CreateInstance(string userid, string username, WFProcessInstanceEntity wfProcessInstanceEntity, string frmData = null)
        {
            try
            {
        ...
        /// <param name="frmData"></param>
        /// <returns></returns>
        public bool EditionInstance(string username, string processId, string description, string frmData = null)
        {
            try
            {
        ...
        /// <param name="processId"></param>
        /// <returns></returns>
        public bool NodeVerification(string userid, string username, string processId, bool flag, string description = "")
        {
            bool _res = false;
            try
            {
                ...
                if (wfProcessInstanceEntity.ActivityType == 0)//会签
                {
                    ...
                    {
                        _makerList = GetMakerList(wfruntime.runtimeModel.nodeDictionary[item], wfruntime.runtimeModel.processId, userid);
                        if (_makerList != "-1")
                        {
                    ...
                    if (_VerificationNodeId != "")
                    {
                        if (flag)
                        {
                            wfProcessOperationHistoryEntity.Content = "【" + username + "】【" + wfruntime.runtimeModel.nodeDictionary[_VerificationNodeId].name + "】【" + DateTime.Now.ToString("yyyy-MM-dd HH:mm") + "】同意,备注：" + description;
                        }
                        else
                        {
                            wfProcessOperationHistoryEntity.Content = "【" + username + "】【" + wfruntime.runtimeModel.nodeDictionary[_VerificationNodeId].name + "】【" + DateTime.Now.ToString("yyyy-MM-dd HH:mm") + "】不同意,备注：" + description;
                        }

                        string _Confluenceres = wfruntime.NodeConfluence(_VerificationNodeId, flag, userid, description);
                        var _data = new
                        {
                        ...
                        switch (_Confluenceres)
                        {
                            ...
                            default://通过
                                ...
                                wfProcessInstanceEntity.MakerList = (wfruntime.runtimeModel.nextNodeType == 4 ? GetMakerList(wfruntime, userid) : "");//当前节点可执行的人信息
                ...
                else//一般审核
                {
                    if (flag)
                    {
                        wfruntime.MakeTagNode(wfruntime.runtimeModel.currentNodeId, 1, userid, description);
                        wfProcessInstanceEntity.PreviousId = wfProcessInstanceEntity.ActivityId;
                        wfProcessInstanceEntity.ActivityId = wfruntime.runtimeModel.nextNodeId;
                        wfProcessInstanceEntity.ActivityType = wfruntime.runtimeModel.nextNodeType;//-1无法运行,0会签开始,1会签结束,2一般节点,4流程运行结束
                        wfProcessInstanceEntity.ActivityName = wfruntime.runtimeModel.nextNode.name;
                        ...
                        wfProcessOperationHistoryEntity.Content = "【" + username + "】【" + wfruntime.runtimeModel.currentNode.name + "】【" + DateTime.Now.ToString("yyyy-MM-dd HH:mm") + "】同意,备注：" + description;
                    }
                    else
                    {
                        ...
                        wfProcessOperationHistoryEntity.Content = "【" + username + "】【" + wfruntime.runtimeModel.currentNode.name + "】【" + DateTime.Now.ToString("yyyy-MM-dd HH:mm") + "】不同意,备注：" + description;
                    }
                    var data = new
                    {
                        SchemeContent = wfruntime.runtimeModel.schemeContentJson.ToString(),
        ...
        /// <param name="description"></param>
        /// <returns></returns>
        public bool NodeReject(string userid, string username, string processId, string nodeId, string description = "")
        {
            try
            {
                WFProcessInstanceEntity wfProcessInstanceEntity = wfProcessInstanceService.GetEntity(processId);
                ...
                wfruntime.MakeTagNode(wfruntime.runtimeModel.currentNodeId, 0, userid, description);
                ...
                if (resnode != "")
                {
                    ...
                    wfProcessInstanceEntity.MakerList = GetMakerList(wfruntime.runtimeModel.nodeDictionary[resnode], wfProcessInstanceEntity.PreviousId, userid);//当前节点可执行的人信息
                    #region 流转记录
                    processTransitionHistoryEntity = new WFProcessTransitionHistoryEntity();
                    processTransitionHistoryEntity.fromNodeId = wfruntime.runtimeModel.currentNodeId;
                    processTransitionHistoryEntity.fromNodeName = wfruntime.runtimeModel.currentNode.name.Value;
                ...
                wfProcessSchemeEntity.SchemeContent = data.ToJson().ToString();
                wfProcessOperationHistoryEntity.Content = "【" + username + "】【" + wfruntime.runtimeModel.currentNode.name + "】【" + DateTime.Now.ToString("yyyy-MM-dd HH:mm") + "】驳回,备注：" + description;
        ...
        /// </summary>
        /// <param name="processId"></param>
        public void CallingBackProcess(string username, string processId)
        {
            try
            {
                wfProcessInstanceService.OperateVirtualProcess(username, processId, 2);
            }
            catch
            {
                throw;
        ...
        /// <returns></returns>
        private string GetMakerList(IWF_Runtime wfruntime, string userid)
        {
            try
            {
                string makerList = "";
                ...
                {
                    makerList = GetMakerList(wfruntime.runtimeModel.nextNode, wfruntime.runtimeModel.processId, userid);
                    if (makerList == "-1")
                    {
        ...
        /// <returns></returns>
        private string GetMakerList(dynamic node, string processId, string userid)
        {
            try
            {
                string makerlsit = "";
