...
//跳转到班组考评打分页面
function dafen(iframeId) {
    var path = document.baseURI;
    path=   path.replace(top.location.origin, "");
    var ind = path.indexOf('/', 2);
    path = path.substring(0, ind);
    $.ajax({
        url: path + "/home/GetModuleInfo",
        data: { moduleName: "考评打分" },
        type: "POST",
        success: function (data) {
            if (data != null) {
                addTabSK(data.ModuleId, path + "/EvaluateAbout/Evaluate/index", "考评打分");//在双控下调用
                //addTabSK("dafen", dafenurl);
                if (id) {
                    dialogClose();
                }
                //addTab("97fefc17-32b4-4027-bc74-ba22523248a9", path + "/EvaluateAbout/Evaluate/index", "考评打分");//在班组下调用
            } else {
                alert("找不到菜单");
            }
        },
