...
(function ($) {
    ...
    $.fn.treeview = function (settings) {
        ...
        var dfop =
            {
                 ...
                 */
                cascadecheck: true,  //设置级联选中状态，若子节点选中则父级节点自动选中
                /**
        ...
        function buildnode(nd, ht, deep, path, isend) {
            ...
            var html = "<span data-value='" + nd.id + "' class='bbit-tree-node-text' unselectable='on'";
            if (nd.Code != undefined) {
                html += " data-code='" + nd.Code + "'";
                //ht.push("<span data-value='" + nd.id + "' class='bbit-tree-node-text' unselectable='on' data-code='" + nd.Code + "'>", nd.text, "</span>");
            }
            if (nd.Dept != undefined) {
                html += " data-dept='" + nd.Dept + "'";
                //ht.push("<span data-value='" + nd.id + "' class='bbit-tree-node-text' unselectable='on' data-code='" + nd.Code + "'>", nd.text, "</span>");
            }
            //else {
            //    ht.push("<span data-value='" + nd.id + "' class='bbit-tree-node-text' unselectable='on' >", nd.text, "</span>");
            ...
            if (dfop.isTool) {
                ...
                for (var ii in dfop.nodeTools) {
                    ...
                    if (toolItem.node == undefined || toolItem.node == nd.type) {
                        ht.push("<span class='" + toolItem.img + "'  data-value='" + nd.id + "' title='" + toolItem.text + "'>" + toolItem.text + "</span>");
                    }
                }
                ht.push("</div>");
            }
        ...
        function getck(items, c, fn) {
            for (var i = 0, l = items.length; i < l; i++) {
                ...
                //if (items[i].Code != undefined && items[i].showcheck && items[i].checkstate ==1) {
        ...
        me[0].t = {
            ...
            getSelectedValues: function () {
                var s = [];
                getck(treenodes, s, function (item) { return item.value = item.Code == undefined ? "," : item.value; });
                return (s + "").replace(/,,/g, "");
            },
            getSelectedTexts: function () {
                var s = [];
                getck(treenodes, s, function (item) { return item.text = item.Code == undefined ? "," : item.text; });
                return (s+"").replace(/,,/g,"");
            },
            getSelectedIds: function () {
                var s = [];
                getck(treenodes, s, function (item) { return item.id = item.Code == undefined ? "," : item.id; });
                return (s + "").replace(/,,/g, "");
            },
