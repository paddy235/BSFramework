(function ($) {
    ...
    $.tab = {
        ...
        addTab: function (dId, href, text) {
            ...
            var menuName = href == undefined ? $.trim($(this).text()) : text;
        ...
        init: function () {
            $('.menuItem').on('click', top.$.tab.addTab);
            $('.menuTabs').on('click', '.menuTab i', top.$.tab.closeTab);
            $('.menuTabs').on('click', '.menuTab', top.$.tab.activeTab);
            $('.tabLeft').on('click', top.$.tab.scrollTabLeft);
            $('.tabRight').on('click', top.$.tab.scrollTabRight);
            $('.tabReload').on('click', top.$.tab.refreshTab);
    ...
    $.index = {
        load: function () {
            $("#content-wrapper").find('.mainContent').height($(window).height() - 128);
        ...
        loadMenu: function () {
                ...
                if (row.ParentId == "0") {
                    if (row.UrlAddress != null && row.UrlAddress != undefined) {
                        if (row.UrlAddress.length > 2) {
                            _html += '<li><a class="menuItem" href="' + contentPath + row.UrlAddress + '" data-id="' + row.ModuleId + '"><i class="fa ' + row.Icon + '"></i><span>' + row.FullName + '</span></a><em></em></li>';
                        }
                        else {
            ...
            $ul.find("li").click(function () {
                ...
                var childNodes = $.index.jsonWhere(data, function (v) { return v.ParentId == dataId });
                var _childNodeshtml = "";
                $.each(childNodes, function (i) {
                    var row = childNodes[i];
                    var subchildNodes = $.index.jsonWhere(data, function (v) { return v.ParentId == row.ModuleId });
                    if (subchildNodes.length > 0) {
                        ...
                        $.each(subchildNodes, function (i) {
                            ...
                            _childNodeshtml += '<li><a class="menuItem" data-id="' + subrow.ModuleId + '" href="' + contentPath + subrow.UrlAddress + '"><i class="fa ' + subrow.Icon + '"></i><span>' + subrow.FullName + '</span></a></li>';
                        });
                        _childNodeshtml += '</ul>';
                        _childNodeshtml += '</li>';

                    } else {
                        _childNodeshtml += '<li><a class="menuItem" data-id="' + row.ModuleId + '" href="' + contentPath + row.UrlAddress + '"><i class="fa ' + row.Icon + '"></i><span>' + row.FullName + '</span></a></li>';
                    }
                });
                ...
                $.tab.init();
                if ($(".sidebar-menu-right").height() > $(".sidebar-menu-left").height()) {
                    $(".sidebar-menu-left").height($(".sidebar-menu-right").height());
                }

            });
            $(".sidebar-menu-left").height(($(window).height() - 200));
        ...
        loadDesktop: function () {
            if ($(".slidebox").length > 0) {
                ...
                $.ajax({
                    url: top.contentPath + "/ClientData/GetMenuDataJson",
                    type: "post",
                    dataType: "json",
                    async: false,
                    success: function (data) {
                        ...
                        $.each(data, function (i) {
                            ...
                            if (row.Target == "iframe") {
                                ...
                                _html += '<li class="menuItem" data-id="' + row.ModuleId + '" href="' + top.contentPath + row.UrlAddress + '">';
                                _html += '    <div class="icon" style="background: #' + color[colorindex] + ';">';
                                _html += '        <i class="fa ' + row.Icon + '"></i>';
                                _html += '     </div>';
                                _html += '     <div class="icon-text">';
                                _html += '         <span>' + row.FullName + '</span>';
                                _html += '     </div>';
                                _html += '</li>';
                            }
                        });
