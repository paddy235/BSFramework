...
var SimpleCalendar = function () {
    ...
    _createClass(SimpleCalendar, [{
        ...
        value: function create() {
            ...
            //actions
            header.innerHTML = header.innerHTML + '<div class="sc-actions">' + '<select class="sc-select-year" name="">' + '      </select>' + '  </div>';
            ...
            header.innerHTML = header.innerHTML + '<div class="sc-actions"><span class="sc-return-today ">回到今天</span></div>';
            ...
            for (var i = 0; i < 35; i++) {
                days.innerHTML = days.innerHTML + '<div class="sc-item" data-method="offset" data-type="auto"  ><div class="day"></div><div class="lunar-day"></div><span class="addMark"></span></div>';
            }
            //添加下拉框数据
        ...
        value: function update() {
            ...
            $('.addMark').each(function (i, item) {
                var day = $(this).prev().prev().html()
                var state = fn$getAttendance(day, !jQuery(item).parent().hasClass('sc-othermenth'));

                if ((i + 1) % 7 == 0 || ((i + 2) % 7 == 0)) {
                    if (!state)
                        $('.addMark').eq(i).html('休息');
                    else {
                        if (state.indexOf('0.5') > 0)
                            $('.addMark').eq(i).html(state + ' 天')
                        else
                            $('.addMark').eq(i).html(state)
                    }
                } else {
                    if (state.indexOf('0.5') > 0)
                        $('.addMark').eq(i).html(state + ' 天')
                        ...
                        $('.addMark').eq(i).html(state)
                    //if (ss == '20') {
                    //    $('.addMark').eq(i).html('sssss')
                    //} else {
                    //    $('.addMark').eq(i).html('出勤')
                    //}

                }

            })
            $('.lunar-day').each(function (i, item) {
                let a = $('.lunar-day').eq(i).html()
                if (a != '') {
                    //    		$('.lunar-day').eq(i).html('休息')
                    $('.addMark').eq(i).html('')
                }
        ...
        value: function updateEvent() {
            ...
            //      calendar.update(this.tmonth, currentyear);
            //    };
            $('.item-nolunar').on('click', function () {
                $(this).find('.addMark').html()
            })
