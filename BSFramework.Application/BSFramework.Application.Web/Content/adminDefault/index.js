...
$.fn.Tab = function (options) {
    ...
    //tab target content
    tabContent = $('<div />').attr('id', 'jquery_tab_div_content').width(cfg.tabcontentWidth).height(cfg.height - cfg.tabHeight).addClass(cfg.tabContentClass);
    ...
    function addTab(item) {
        ...
        if (tab.find("#" + item.id).length == 0) {
            ...
            liObj.append(innerString).appendTo(tab.find('ul')).find('table td:eq(1)').click(function () {
                ...
                cfg.currentEvent(liObj.find('.inner').attr('data-value'), item.title);
            })
            if (item.url) {
                var $iframe = $("<iframe class=\"desktop_iframe\" id=\"iframe" + item.id + "\" name=\"iframe" + item.id + "\"  height=\"100%\" width=\"100%\" src=\"" + item.url + "\" frameBorder=\"0\"></iframe>")
    ...
    function newTab(item) {
        ...
        if (item.moduleIdCookie == true) {
            ...
            top.$.cookie('currentmoduleName', item.title, { path: "/" });
...
loadnav = function () {
    ...
    tablist = $("#tab_list_add").Tab({
        items: [
            { id: 'desktop', title: '首页', closed: false, icon: 'fa fa fa-desktop', url: contentPath + '/Home/AdminDefaultDesktop' },
        ],
        tabScroll: false,
        ...
        currentEvent: function (moduleId, moduleName) {
            top.$.cookie('currentmoduleId', moduleId, { path: "/" });
            top.$.cookie('currentmoduleName', moduleName, { path: "/" });
        }
    });
    //个人中心
    $("#UserSetting").click(function () {
        tablist.newTab({ id: "UserSetting", title: "个人设置", closed: true, icon: "fa fa fa-user", url: contentPath + "/PersonCenter/Index" });
    });
    $("#HomePage").click(function () {
        tablist.newTab({ id: "desktop", title: "首页", closed: false, icon: "fa fa fa-desktop", url: contentPath + "/Home/Desktop" });
    });
    //动态加载导航菜单
    var _html = "";
    var index = 0;
...
//安全退出
function IndexOut() {
    dialogConfirm("注：您确定要安全退出本次登录？", function (r) {
        if (r) {
            Loading(true, "正在安全退出中...");     
            window.setTimeout(function () {
                $.ajax({
