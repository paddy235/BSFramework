...
namespace BSFramework.Application.Web.Areas.SystemManage.Controllers
{
    ...
    public class DataItemDetailController : MvcControllerBase
    {
        ...
        [HttpGet]
        public ActionResult GetTeamTypeListJson(string EnCode)
        {
            var data = dataItemDetailBLL.GetDataItemList().Where(x => x.EnCode == EnCode); //dataItemCache.GetDataItemList(EnCode);
            return Content(data.ToJson());
        }
        ...
        public ActionResult GetSubDataItemListJson(string EnCode, string ItemValue)
        {
            ...
            var data1 = data.First(x => x.EnCode == EnCode && x.ItemValue == ItemValue); //dataItemCache.GetSubDataItemList(EnCode, ItemValue);
            data = data.Where(x => x.ParentId == data1.ParentId);
            return Content(data.ToJson());
        }
        #endregion
