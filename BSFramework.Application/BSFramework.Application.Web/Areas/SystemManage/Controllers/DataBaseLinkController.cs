...
namespace BSFramework.Application.Web.Areas.SystemManage.Controllers
{
    ...
    public class DataBaseLinkController : MvcControllerBase
    {
        ...
        [ValidateAntiForgeryToken]
        [AjaxOnly]
        [HandlerMonitor(5, "保存库连接表单(新增、修改)")]
        public ActionResult SaveForm(string keyValue, DataBaseLinkEntity databaseLinkEntity)
        {
            ...
            //}
            if (databaseLinkEntity.DbConnection.Contains("*"))
            {
                DataBaseLinkEntity data = databaseLinkBLL.GetEntity(keyValue);
                databaseLinkEntity.DbConnection = data.DbConnection;
            }
