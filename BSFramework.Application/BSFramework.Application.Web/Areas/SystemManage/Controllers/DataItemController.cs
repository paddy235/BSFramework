...
namespace BSFramework.Application.Web.Areas.SystemManage.Controllers
{
    ...
    public class DataItemController : MvcControllerBase
    {
        ...
        {
            var entity = dataItemBLL.GetEntityByName(keyValue);
            var list = new DataItemDetailBLL().GetList(entity.ItemId);
            var select = list.Select(x => new SelectListItem() { Value = x.ItemValue, Text = x.ItemName });
            return Content(select.ToJson());
        }
        #endregion
