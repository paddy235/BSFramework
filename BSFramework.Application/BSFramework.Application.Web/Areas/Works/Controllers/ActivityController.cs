...
namespace BSFramework.Application.Web.Areas.Works.Controllers
{
    ...
    public class ActivityController : MvcControllerBase
    {
        ...
        public ViewResult Detail4(string id)
        {
            var user = OperatorProvider.Provider.Current();
            var bll = new ActivityBLL();
            ...
            if (data.Files != null)
            {
                ...
                {
                    if (item.FileType == "java_doc" || item.FileType == "java_video")
                    {
                        var url = GetResourceUrl(item.FolderId, user.UserId);
                        if (!string.IsNullOrEmpty(url)) item.FilePath = url;
                    }
                }
            }
            return View(data);
        ...
        public ViewResult Form(string id)
        {
            ...
            var activityAcitonList = new ActivityActionBLL().GetListByActionId(id, model.GroupId);
            ViewBag.ActivityAcitonList = activityAcitonList;
            ...
            if (model.Files !=null && model.Files.Count>0)
            {
                foreach (var item in model.Files)
                {
                    switch (Path.GetExtension(item.FileName).ToLower())
                    {
                        ...
                        case ".ppt":
                        case ".pptx":
                            if (System.IO.File.Exists( Server.MapPath("~/Resource/ActivityPDF/" + item.FileId + ".pdf")))
                            {
                                item.ViewUrl = "~/Resource/ActivityPDF/" + item.FileId + ".pdf";
                                item.CanView = true;
                            }
                            else {
                                using (var factory = new ChannelFactory<IQueueService>("upload"))
                                {
                                    ...
                                    channel.OfficeToPdf(Server.MapPath(item.FilePath), Server.MapPath("~/Resource/ActivityPDF/" + item.FileId + ".pdf"));
                                }
                            }
                            break;
                        case ".pdf":
        ...
        public ActionResult GetActivityPageList(Pagination pagination, string queryJson, string deptid, string category, string select, string isEvaluate)
        {
            ...
            //				 ELSE 0
            //				 END
            //				 as number, case when d.dc > 0 then 1 else 0 end as hasSub";
            ...
            //                    left join (
            //select ActivityId, count(1) as dc from wg_subactivity
            //group by ActivityId ) d on d.ActivityId = a.activityid";
            //            pagination.conditionJson = "1=1";
            ...
            ActivityBLL actBll = new ActivityBLL();
            var data = actBll.GetActivityPageList(pagination, queryJson, select, deptid, category, isEvaluate, user.UserId);
        ...
        /// </summary>
        /// <returns></returns>
        public ActionResult IndexActivityPartView()
        {
            return View();
        }
        #region 获取机构部门组织树菜单
        ...
        public ActionResult getjobs(string userid, string deptid, string jobname, int checkMode = 0, int mode = 0)
        {
            ...
            var data = getNewData(userid, deptid).Distinct(new DuplicateDefine());
        ...
        public JsonResult GetTimeCount(string from, string to, string category)
        {
            ...
            var deptListALL = departmentBLL.GetSubDepartments(dept.DepartmentId, "");
            var deptStr = string.Join(",", deptListALL.Select(x => x.DepartmentId));
        ...
        /// <summary>
        /// 查询月度、季度、年度 各类型班组活动的开展次数
        /// </summary>
        /// <param name="dateType">月度、季度、年度</param>
        /// <returns></returns>
        [HttpGet]
        public ActionResult ActivityTypeCount(string dateType)
        {
            ActivityBLL bll = new ActivityBLL();

            Operator user = OperatorProvider.Provider.Current();
            var company = departmentBLL.GetCompany(user.DeptId);//获取当前登录人所在的电厂
            var subDept = departmentBLL.GetSubGroups(company.DepartmentId);//获取该电厂下所有的班组
            ...
            var data = bll.ActivityTypeCount(from, to, subDept.Select(x => x.DepartmentId).ToList())
                  .Select(p => new
                  {
                      name = p.key,
        ...
        public JsonResult GetSubjects(int rows, int page)
        {
            var bll = new ActivitySubjectBLL();
            var total = 0;
            var data = bll.GetSubjects(rows, page, out total);

            return Json(new { rows = data, total = Math.Ceiling((decimal)total / rows), page = page, records = total }, JsonRequestBehavior.AllowGet);
        }

        public ViewResult EditSubject(string id)
        {
            var list1 = new DataItemDetailBLL().GetDataItems("议题类型");
            var list2 = new DataItemDetailBLL().GetDataItems("议题状态");

            ViewData["SubjectType"] = new SelectList(list1, "ItemName", "ItemName");
            ViewData["Status"] = new SelectList(list2, "ItemName", "ItemName");
            var bll = new ActivitySubjectBLL();

            var model = default(ActivitySubjectEntity);
            if (string.IsNullOrEmpty(id))
                model = new ActivitySubjectEntity();
            else
                model = bll.Get(id);
        ...
        [HttpPost]
        public ActionResult EditSubject(string id, ActivitySubjectEntity activitySubjectEntity)
        {
            if (string.IsNullOrEmpty(activitySubjectEntity.ActivitySubjectId)) activitySubjectEntity.ActivitySubjectId = Guid.NewGuid().ToString();
            var bll = new ActivitySubjectBLL();
            bll.EditSubject(activitySubjectEntity);
            return Success("保存成功！");
        }
        ...
        {
            var bll = new ActivitySubjectBLL();
            bll.DeleteSubject(id);
            return Success("删除成功！");
        }

        public JsonResult GetSubActivities(string id)
        {
            var bll = new ActivityBLL();
            var data = bll.GetDetail(id);
            return Json(data.SubActivities, JsonRequestBehavior.AllowGet);
        }

        public string GetResourceUrl(string fileid, string userid)
        {
            var url = "api/v1.0/course/selectUrlByFileId";
            string userId = OperatorProvider.Provider.Current().UserId;
            var client = new HttpClient();
            HttpContent httpheard = new StringContent("{\"file_id\":\"" + fileid + "\"}");
            ...
            if (data.HasValues) return data.Value<string>("url");
            else return string.Empty;
        }

        public ContentResult Yanlian(string id)
        {
            var user = OperatorProvider.Provider.Current();
            var obj = new { userid = user.UserId, data = new { keyvalue = id } };
            var dict = new Dictionary<string, string>();
            dict.Add("json", Newtonsoft.Json.JsonConvert.SerializeObject(obj));

            var client = new HttpClient();
            var response = client.PostAsync(Config.GetValue("ErchtmsApiUrl") + "EmergencyPlatform/GetDrillRecordBase", new FormUrlEncodedContent(dict)).Result;
        ...
        /// 根据配置获取页面展示
        /// </summary>
        public JsonResult GetActivityMenu(string module)
        {
            var user = OperatorProvider.Provider.Current();
            var bll = new ActivityBLL();
            var data = bll.GetActivityMenu(user.UserId, user.DeptId, module);
            return Json(data);
        }
