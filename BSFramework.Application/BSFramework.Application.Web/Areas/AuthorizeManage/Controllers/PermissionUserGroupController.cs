    ...
    public class PermissionUserGroupController : MvcControllerBase
    {
        ...
        public ActionResult GetUserListJson(string userGroupId)
        {
            ...
            if (userdata != null && userdata.Count > 0)
            {
                ...
                {
                    int ischeck = existMember.Count(t => t.UserId == x.UserId);
                    int? IsDefault = 0;
                    if (ischeck > 0)
                    {
        ...
        [AjaxOnly]
        [HandlerMonitor(5, "保存(新增或修改)用户组成员信息")]
        public ActionResult SaveMember(string userGroupId, string userIds)
        {
            permissionBLL.SaveMember((int)AuthorizeTypeEnum.UserGroup, userGroupId, userIds);
            return Success("保存成功。");
        }
        /// <summary>
        /// 保存用户组授权
        ...
        [ValidateAntiForgeryToken]
        [AjaxOnly]
        [HandlerMonitor(5, "保存(新增或修改)用户组授权信息")]
        ...
        {
            permissionBLL.SaveAuthorize((int)AuthorizeTypeEnum.UserGroup, userGroupId, moduleIds, moduleButtonIds, moduleColumnIds, authorizeDataJson);
            return Success("保存成功。");
        }
        #endregion
    }
