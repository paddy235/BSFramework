...
{
    public class EducationWarning:WarningBase
    {
        public EducationWarning(string messagekey)
            : base(messagekey)
        {
        }
        ...
        public override string[] FindWaringData()
        {
            ...
            var query = from q in db.IQueryable<EduBaseInfoEntity>()
                        where q.ActivityEndDate >= @from && q.ActivityEndDate <= to && (q.EduType == "5" ) && q.AnswerFlow == "0"
                        select q;
