...
namespace BSFramework.Data.Repository
{
    ...
    public class Repository<T> : IRepository<T> where T : class,new()
    {
        ...
        public IEnumerable<T> FindListByProcPager(Pagination pagination, DatabaseType dataType) 
        {
            var data = db.FindListByProcPager<T>(pagination, dataType); 

            return data;

        }
        public DataTable FindTableByProcPager(Pagination pagination, DatabaseType dbType)
        {
            return db.FindTableByProcPager(pagination, dbType);
        }
    }
}
