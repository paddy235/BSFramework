...
namespace BSFramework.Util.WebControl
{
    /// <summary>
    /// 构造树形Json
    /// </summary>
    public static class TreeJson
    {
        ...
        public static string TreeToJson(this List<TreeEntity> list, string ParentId = "0")
        {
            ...
            List<TreeEntity> item = new List<TreeEntity>();
            if (ParentId.Contains(","))
            {
                List<string> list1 = new List<string>();
                foreach (string str in ParentId.Split(','))
                {
                    list1.Add(str);
                }
                item = list.FindAll(t => list1.Contains(t.parentId));
            }
            else
            {
                item = list.FindAll(t => t.parentId == ParentId);
            }
            strJson.Append("[");
            if (item.Count > 0)
            {
