...
namespace BSFramework.Data.Dapper
{
    /// <summary>
    /// 描 述：操作数据库
    /// </summary>
    public class OracleDatabase : IDatabase
    {
        ...
        protected DbConnection Connection
        {
            get
            {
                ...
                DbConnection dbconnection = Activator.CreateInstance(type, connectionString) as DbConnection;
        ...
        public int ExecuteBySql(string strSql, params DbParameter[] dbParameter)
        {
            ...
            foreach (DbParameter pm in dbParameter)
            {
                dic.Add(pm.ParameterName, pm.Value);
            }
            if (dbTransaction == null)
            {
                using (var connection = Connection)
                {
                    return connection.Execute(strSql, dic);
                }
            }
            else
            {
                dbTransaction.Connection.Execute(strSql, dic, dbTransaction);
                return 0;
        ...
        public int ExecuteByProc(string procName, params DbParameter[] dbParameter)
        {
            ...
            if (dbTransaction == null)
            {
                using (var connection = Connection)
                {
                    return connection.Execute(procName, dic);
                }
            }
            else
            {
                dbTransaction.Connection.Execute(procName, dic, dbTransaction);
                return 0;
        ...
        public IEnumerable<T> FindList<T>(string strSql, DbParameter[] dbParameter) where T : class
        {
            ...
            using (var dbConnection = Connection)
            {
                return dbConnection.Query<T>(strSql, dic);
            }
        }
