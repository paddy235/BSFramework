...
namespace BSFramework.Util.Offices
{
    ...
    public class ExcelHelper
    {
        ...
        public static void ExcelDownload(DataTable dtSource, ExcelConfig excelConfig)
        {
            ...
            {//兼容FireFox
             //  fileName = "=?UTF-8?B?" + Convert.ToBase64String(System.Text.Encoding.UTF8.GetBytes(excelConfig.FileName)) + "?=";
                fileName = excelConfig.FileName;
            }
            else
            {
                fileName = HttpContext.Current.Server.UrlEncode(excelConfig.FileName);
            }
        ...
        public static void DownLoadFile(string sourceFullName, string downloadName = "")
        {
            ...
            try
            {
                if (!File.Exists(sourceFullName)) throw new Exception("文件不存在或已被删除!");
                ...
                // 纯下载方式
                var userAgent = HttpContext.Current.Request.Headers["User-Agent"];
                if (!string.IsNullOrEmpty(userAgent) && userAgent.ToLower().Contains("firefox"))
                {//兼容FireFox
                    fn = "=?UTF-8?B?" + Convert.ToBase64String(System.Text.Encoding.UTF8.GetBytes(fn)) + "?=";
                }
                else
                {
                    fn = System.Web.HttpUtility.UrlEncode(fn);
                }
        ...
        {
            int colint = 0;
            if (excelConfig.ColumnEntity.Count > 0)
            {
                for (int i = 0; i < dtSource.Columns.Count;)
                {

                    DataColumn column = dtSource.Columns[i];
                    if (colint >= excelConfig.ColumnEntity.Count)
                    {
                        colint = excelConfig.ColumnEntity.Count - 1;
                    }
            ...
            #region 右击文件 属性信息
            {
                ...
                si.ApplicationName = "应用程序"; //填加xls文件创建程序信息
            ...
            foreach (DataRow row in dtSource.Rows)
            {
                ...
                if (rowIndex == 65535 || rowIndex == 0)
                {
                    ...
                    #region 列头及样式
                    {
                        ...
                        headerRow.Height = 25 * 25;
                        //headerRow.RowStyle.Alignment = HorizontalAlignment.Center;
                        #endregion
                    }
                    #endregion
